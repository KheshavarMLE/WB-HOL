...
import {
  ...
  Wrench, Database, ArrowRight, Share2, Copy, Check,
} from 'lucide-react';
import { encodeSessionSnapshot } from '@/hooks/useSession';
...
const SessionSelector = () => {
  ...
  const handleShareLink = async () => {
    if (!session) return;
    const snap = encodeSessionSnapshot(session as any);
    const url = `${window.location.origin}/session/${sessionId}?snap=${snap}`;
    try {
      await navigator.clipboard.writeText(url);
      setCopied(true);
      toast.success('Session link copied! Anyone with this can open your session.');
